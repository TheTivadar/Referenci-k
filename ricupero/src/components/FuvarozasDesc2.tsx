...
interface Props {
    utepitesSzolgaltatas2: KontenerTipus[];
}

const FuvarozasDesc2 = ({ utepitesSzolgaltatas2 }: Props) => {
  if (!utepitesSzolgaltatas2) return null;
  const title1 = utepitesSzolgaltatas2.find(item =>item.id === "1")?.title;
  const body2 = utepitesSzolgaltatas2.find(item => item.id === "2")?.body;
  const title2 = utepitesSzolgaltatas2.find(item =>item.id === "2")?.title;
  const body3 = utepitesSzolgaltatas2.find(item => item.id === "3")?.body;
  const title3 = utepitesSzolgaltatas2.find(item =>item.id === "3")?.title;
  const body4 = utepitesSzolgaltatas2.find(item => item.id === "4")?.body;
  const title4 = utepitesSzolgaltatas2.find(item =>item.id === "4")?.title;
