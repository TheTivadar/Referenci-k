...
interface Props {
    gepparkGarantalunk: KontenerTipus[];
}


const GepekFeature = ({ gepparkGarantalunk }: Props) => {
  if(!gepparkGarantalunk) return null;
  const title1 = gepparkGarantalunk.find(item =>item.id === "1")?.title;
  const body2 = gepparkGarantalunk.find(item => item.id === "2")?.body;
  const title2 = gepparkGarantalunk.find(item =>item.id === "2")?.title;
  const body3 = gepparkGarantalunk.find(item => item.id === "3")?.body;
  const title3 = gepparkGarantalunk.find(item =>item.id === "3")?.title;
  const body4 = gepparkGarantalunk.find(item => item.id === "4")?.body;
  const title4 = gepparkGarantalunk.find(item =>item.id === "4")?.title;
  ...
  return (
    <div className="md:pt-20 relative px-6  lg:overflow-visible lg:px-0">
    ...
    <div className="pb-16 grid grid-cols-1 space-y-4  md:space-y-0 sm:grid-cols-3  relative z-10  max-w-full mx-auto">
